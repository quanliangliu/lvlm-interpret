...
def process_image(image, image_process_mode, return_pil=False, image_format='PNG', max_len=1344, min_len=672, is_mllama=False):
    # For Mllama models, use smaller max resolution to avoid OOM in vision attention
    if is_mllama:
        max_len = min(max_len, 560)  # Limit to 560px to reduce memory usage
        min_len = min(min_len, 560)
